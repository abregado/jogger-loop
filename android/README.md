...
- **Release APK filename**: renamed from Gradle's default `app-release.apk` to `jogger-loop-<versionName>.apk` via an `androidComponents { onVariants(...) }` block in `android/app/build.gradle.kts`, scoped to the release build type only (debug keeps the default). The release workflow's file-upload steps already glob `*.apk`, so this needed no CI changes.
